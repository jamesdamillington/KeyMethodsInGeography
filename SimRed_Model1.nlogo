;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;                                                                              ;;
;;              Vegetation-Moisture Feedback 1 - May 2015                       ;;
;;                                                                              ;;
...
to go
  ...
  ;if there are no plants, this is the first plant to grow
  ifelse(not any? plants)
  [
    ask patch 0 0  ;ask the centre patch to sprout the first plant (regardless of soil conditions)  
    [ 
      sprout-plants 1       ;create the plant  
      [ 
        set shape "circle"  ;make it a circle shape
        set color green     ;give it a green colour 
      ]
    ]
  ]

  ;otherwise, if there are already some plants, grow veg patch
  [
    let grow-location nobody  ;this is an agentset that will contain the patch where the next plant should grow

    ;if user has selected 'random' establishment, set the grow-location patch to be a radomly selected patch without any plants and a positive infiltration rate
    if(establishment = "Random") [ set grow-location one-of patches with [not any? plants-here and infiltration-rate > 0]] 
    
    ;if the user has selected 'wettest' establishment, set grow-location to the patch with the greatest infiltration-rate 
    if(establishment = "Wettest") [ set grow-location max-one-of patches with [not any? plants-here and count plants-on neighbors > 0] [ infiltration-rate ]]
  ...
  calc-plant-density        ;run the calc-plant-density procedure
  calc-infiltration-rate   ;run the calc-infiltration-rate procedure
  set-patch-colours         ;run the set-patch-colours procedure

  wait 0.5  ;pause to view growth easier
  
  if(count plants >= 80) [ stop ]  ;stop simulation when there are 80 plants
...
to calc-plant-density
  ...
  ask patches
  [
    let neighbour-plants plants-on neighbors           ;create an agentset containing any plant agents on the 8 neighbouring patches of this patch
    let count-neighbour-plants count neighbour-plants  ;count the number of agents in the neighbour-plants agentset and put it in the count-neighbour-plants variable
    if any? plants-here [ set count-neighbour-plants count-neighbour-plants + 1 ] ;if there's also a plant on this particular patch, increase the value of count-neighbour-plants by one
    
    ifelse (count-neighbour-plants > 0)                
    [ set plant-density count-neighbour-plants / 9 ]   ;if there is more than one plant in this nehbourhood of nine patches, calculate plant-density
    [ set plant-density 0 ]                            ;otherwise, set plant-desity to zero
  ]

end  
  
to calc-infiltration-rate

  ask patches [ set infiltration-rate 100 * plant-density ]  ;infiltration-rate is simply plant-density re-scaled from a value 0-1 to a value 0-100
...
to set-patch-colours
  ...
  [
    ...
    [ set pcolor 100 - (infiltration-rate / 10) ]  ;if infiltration-rate of this patch is greater than zero, make it a blue colour with shade scaled by infiltration-rate value
    [ set pcolor brown ]                            ;otherwise set patch colour to brown
  ]
...
One of three rules can be used to determine where a new plant will grow in each timestep (_establishment_):
1. **Wettest:**  on one of the patches with the greatest water availability that doesn't already have a plant growing there
2. **Random:** could establish randonly with equal probability on any patch with sufficient water availability (and no existing plants)

## HOW TO USE IT

First, decide which _establishment_ rule you wish to use. 

To vary patterns between simulation runs change the random number seed using _rand-seed_.

When you are ready to go, click _setup_, then _go_ (if you wish to run one timestep at a time click _step_ instead of _go_).

## THINGS TO NOTICE

Note how the pattern of growth changes for the different _establishment_ rules.  

## THINGS TO TRY

See how patterns vary for different _establishment_ rules and random number generator seeds (i.e., different values of _rand-seed_).
